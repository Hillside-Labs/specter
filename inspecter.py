...
class OpenAPIMinifierService:
    ...
    def create_full_spec(self, open_api_specs):
        # folder_path = f"./outputs/open_api_spec/full_spec"
        ...
        #     yaml.dump(merged_open_api_spec, output_file, indent=4)
    ...
    def minify(self, open_api_spec):
        ...
        for path, methods in open_api_spec["paths"].items():
            for method, endpoint in methods.items():
                ...
                # self.api_url_format = 'https://apidoc.tatum.io/tag/{tag}#operation/{operationId}'
    ...
    def create_key_point_guide(self, minified_endpoints):
        ...
        #     yaml_content = yaml.safe_load(yaml_file)
        prompt_template = ""
        folder_path = f"./outputs/open_api_spec/keypoint"
        # Ensure output directory exists
        os.makedirs(folder_path, exist_ok=True)
        # Define output file path
    ...
    def compare_chunks(self, data_source, document_chunks):
        folder_path = f"./outputs/open_api_spec/endpoints"
...
mini = OpenAPIMinifierService()
endpoints, ep_by_method = mini.run(
